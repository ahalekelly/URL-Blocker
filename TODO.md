...
Make the displayed times round to the nearest minute - Updated options and stats duration display to whole-minute rounding, including stats summary, domain rows, device totals, and screen-time log rows; added coverage for 90 seconds displaying as 2m.
The sign in buttons are still shown after we're signed in, on both Vivaldi and iOS. And on iOS there's a floating header on the bottom so you don't need the sign in buttons at all - Made hidden sync action buttons stay visually hidden even when provider-button CSS sets display, taught iOS sync status to use the native sign-in-required state on load, and added regression coverage for signed-in and iOS options states. Brave's automated install target could not reload because that profile is not installed from this repo's build path, so Brave was verified in its existing running URL Blocker profile after the standard install.

To Do:
Put commit hashes on the completed tasks in this file
